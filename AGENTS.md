...
- Managed preferences own site policy. Validate them before starting AAC, and do not create or load the browser until `assessmentSessionDidBegin` confirms the session is active.
- AAC is the lockdown boundary. Do not add a fallback lockdown system, elevated window levels, event taps, process control, or PPPC dependencies; the DEBUG unlocked path must not exist in Release builds.
- Release automation produces a signed and notarized Developer ID app inside a signed and notarized installer package. The package installs the app and its supervised LaunchAgent.
